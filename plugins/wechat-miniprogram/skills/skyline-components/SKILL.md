...
description: Skyline 组件开发：scroll-view 增强模式、swiper 布局、表单组件、共享元素动画、半屏拖拽面板等。
---
...
### 横向滚动需同时开启 enable-flex 以兼容 WebView
...
不要在 scroll-view 外直接放置 list-view/grid-view，也不要在 list-view 中使用非直接子节点列表项。
...
### 不要在 Skyline 中使用 web-view/editor/movable-view 组件

### 不要依赖 image 的 WebView-only 裁剪模式（top/bottom/center/left/right）
