---
name: skyline-wxss
description: Skyline WXSS 样式支持：CSS 属性兼容性、不支持的属性与值、渐变/滤镜/遮罩限制。
---
...
.container {
  ...
  position: relative;      /* sticky 不支持，需使用 sticky-header 组件 */
  overflow: hidden;        /* auto 不支持，滚动用 scroll-view */
}
```
...
### text-overflow 仅在 text 组件生效

`text-overflow: ellipsis` 在 `<view>` 上不生效。推荐使用 `<text>` 组件的 `overflow` 和 `max-lines` 属性。

### filter/backdrop-filter 不支持 url() 和 drop-shadow()，也不支持多个函数组合

用 `box-shadow` 替代 `drop-shadow`。支持的函数：`blur()`, `brightness()`, `contrast()`, `grayscale()`, `hue-rotate()`, `invert()`, `opacity()`, `saturate()`, `sepia()`。

### text-decoration-line 仅支持单值

仅支持 `none`, `underline`, `overline`, `line-through`，不支持多值组合如 `underline line-through`。
...
| `text-overflow` | `clip`, `ellipsis`（仅 text 组件生效） | -- |
