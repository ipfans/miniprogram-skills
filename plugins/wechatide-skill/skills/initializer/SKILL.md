...
`login` 只表示二维码已就绪，不代表已登录。返回 `pending + taskId` 后按根 SKILL「异步任务（全局）」与 [异步任务与轮询](../wechatide-tools/references/async-task-polling.md) **主动轮询**（`login` 为会阻塞后续的派生任务）。仅当最终为 `success` 才继续；其他终态停止并如实告知用户。
