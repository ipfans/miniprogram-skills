...
| 页面滚动；写入自动化脚本的截图 | `automation_viewport_action`（`pageScrollTo` / `screenshot`；截图**主归属本 scene**；不传 `path` 返回临时文件路径） |
...
- timeout：记录步骤 → 尽量补当前页/元素/最后成功动作证据

项目窗口未开时先经 initializer。
